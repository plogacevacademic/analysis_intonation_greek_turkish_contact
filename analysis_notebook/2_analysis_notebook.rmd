...
for (cur_dv in dvs)
{
  ...
  loo_compare(fit_mix, fit_baseline)
...
calculate_lambda_by_generation <- function(fit_mix)
{
  ...
  lambda_by_generation <- 
    sapply(1:nrow(df_contr_generation), function(i_row) {
      ...
      credible_interval <- coda::HPDinterval(as.mcmc(cur_samples), prob = .95) %>% c(coda::HPDinterval(as.mcmc(cur_samples), prob = .5)) %>% c(median(cur_samples))
      names(credible_interval) <- c("lower90", "upper90", "lower50", "upper50", "median")
...
fit_mix_fnames <- sprintf("../workspace/fit_mixture_%s.rds", dvs)
df_fit_mix_fnames <- data.frame(dv = dvs, fname = fit_mix_fnames, stringsAsFactors = F) 

lambdas <-
df_fit_mix_fnames %>% plyr::ddply("dv", function(df) {
    #print(df$fname)
    fit_mix <- readRDS(df$fname)
    calculate_lambda_by_generation(fit_mix)
})
...
p <- lambdas %>%
  ggplot(aes(median, generation)) + 
  geom_errorbarh(aes(xmin = lower90, xmax = upper90), height = 0, color = "grey30") + 
  geom_errorbarh(aes(xmin = lower50, xmax = upper50), height = 0, size = 2, color = "grey30") + 
  geom_point(size = 2) +
  theme_bw() + ylab(NULL) + xlab("Estimate") + facet_wrap(~dv, nrow = 1)
print(p)

ggsave(p, file = "../analysis_output/lambda_by_generation.jpeg", width = 10, height = 3)
